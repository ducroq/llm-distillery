...
- **gpu-server deploy** (2026-04-29) — #52 prefilter migrations + review-battery follow-ups (RIP guard repair, multilingual `\b` boundary sweep, CD v4 truncation, etc.) live in production. Deploy created `investment-risk/v6/` (hyphen) on gpu-server without model weights; existing v6 weights live at `investment_risk/v6/model/` (underscore). Yesterday's gotcha-log claimed a band-aid symlink was applied 14:04 UTC but the `ln -s` was never executed — gpu-server scorer ran ~4.5h on warm config then died on next restart cycle (Apr 29 18:34 local), failing 5 consecutive cron triggers and stalling the ovrnews-summarize chain ~13h overnight. **Symlink actually applied 2026-04-30 05:48 UTC**: `investment-risk/v6/model -> ../../investment_risk/v6/model`; scorer restarted, `/health` returns 200. Proper fix still tracked in #53 (NexusMind filter-discovery normalization). <!-- verify: ssh gpu-server "test -L /home/hcl/NexusMind/filters/investment-risk/v6/model && test -r /home/hcl/NexusMind/filters/investment-risk/v6/model/adapter_model.safetensors && curl -fs http://localhost:8000/health > /dev/null" && echo PASS || echo FAIL -->
