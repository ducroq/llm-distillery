...
- **gpu-server filter discovery dedup** (2026-04-30, NexusMind 2d3c666) — #53 STRUCTURALLY FIXED. `FilterLoader.discover_filters()` now collapses hyphen/underscore variant collisions to one registered entry (winner = most complete artifacts), with an alias map so both API name forms resolve. Replaces the symlink band-aid that caused two outages in 24h (13h overnight + ~2h afternoon). Symlink at `investment-risk/v6/model` is now gone (rsync ate it on the deploy that landed this fix) and is no longer load-bearing — discovery picks `investment_risk` directly because the weights are there, ignores the empty `investment-risk/` dir. API calls to either `/filter/investment-risk/score` or `/filter/investment_risk/score` both route correctly. Post-deploy smoke test passed all 7 filters. <!-- verify: ssh gpu-server "curl -fs http://localhost:8000/health > /dev/null && curl -fs http://localhost:8000/models/status | python3 -c 'import sys,json; d=json.load(sys.stdin); names=set(d[\"available_filters\"]); assert len(names)==7 and \"investment_risk\" in names and \"investment-risk\" not in names, names; print(\"OK\")'" && echo PASS || echo FAIL -->
