...
- **gpu-server deploy** (2026-04-29) — #52 prefilter migrations + review-battery follow-ups (RIP guard repair, multilingual `\b` boundary sweep, CD v4 truncation, etc.) live in production. Deploy hit a hyphen/underscore path divergence on investment-risk v6 (briefly took scorer down); band-aid symlink `investment-risk/v6/model -> ../../investment_risk/v6/model` in place. Proper fix tracked in #53. <!-- verify: ssh gpu-server "curl -fs http://localhost:8000/health > /dev/null && grep -q _uppercase_rip_in_title /home/hcl/NexusMind/filters/belonging/v1/prefilter.py" && echo PASS || echo FAIL -->
