...
### CRITICAL EXAMPLES - What Qualifies as "Real Work"?

#### PROPOSAL vs PILOT (MOST COMMON ERROR):
- ❌ **PROPOSAL:** "Company proposes 600 MW battery storage, delivery 2027" → deployment_maturity = 1-2 (future only, no current operation)
- ❌ **PROPOSAL:** "Plans to deploy solar farm in 2026" → deployment_maturity = 1-2 (future commitment, not operational)
- ❌ **ANNOUNCEMENT:** "Startup announces breakthrough battery, production starts 2026" → deployment_maturity = 1-2 (vaporware, no real work yet)
- ✅ **PILOT:** "Pilot battery storage generates 5 MW for 6 months" → deployment_maturity = 3-4 (currently operating with performance data)
- ✅ **PILOT:** "Demonstration plant operates for 12 months, validates 95% efficiency" → deployment_maturity = 4-5 (working pilot with data)
...
- ❌ **OUT OF SCOPE:** "AWS deployment of Python Flask API" → deployment_maturity = 1-2 (generic IT infrastructure, not climate-focused)
- ❌ **OUT OF SCOPE:** "Kubernetes cluster for DevOps team" → deployment_maturity = 1-2 (IT infrastructure, not sustainable tech)
- ❌ **OUT OF SCOPE:** "GitHub Copilot adoption across engineering team" → deployment_maturity = 1-2 (software tools, not climate tech)
- ✅ **IN SCOPE:** "Solar farm management API deployed for 100 MW facility" → Score the SOLAR FARM (6-7), not the API
- ✅ **IN SCOPE:** "EV charging network monitoring system for 500 stations" → Score the EV INFRASTRUCTURE (5-6), not the monitoring system
- ✅ **IN SCOPE:** "Battery storage optimization software for 50 MW BESS" → Score the BATTERY STORAGE (5-6), not the software
...
- ❌ **OUT OF SCOPE:** "Medicinal plants ethnobotanical survey" → deployment_maturity = 1-2 (biodiversity research, not climate tech)
- ❌ **OUT OF SCOPE:** "Traditional ecological knowledge documentation" → deployment_maturity = 1-2 (cultural research, not climate tech)
- ❌ **OUT OF SCOPE:** "Generic conservation biology study" → deployment_maturity = 1-2 (biodiversity, not climate/energy)
- ❌ **OUT OF SCOPE:** "Wildlife habitat restoration project" → deployment_maturity = 1-2 (conservation, not climate tech unless explicitly climate-focused)
- ✅ **IN SCOPE:** "Forest carbon sequestration measurement system deployed" → deployment_maturity = 4-5 (climate tech with real data)
- ✅ **IN SCOPE:** "Ecosystem-based climate adaptation pilot" → deployment_maturity = 3-4 (climate-focused with validation)
- ✅ **IN SCOPE:** "Nature-based carbon removal pilot validates 10 tons CO2/year" → deployment_maturity = 4-5 (climate tech with impact data)
...
**v2.0 (2025-11-18):**
- **CRITICAL FIX: Enhanced Scope Distinction Examples**
  - Added comprehensive PROPOSAL vs PILOT examples (most common error from v1)
  - Added IT DEPLOYMENT vs SUSTAINABLE TECH examples (scope error from v1)
  - Added BIODIVERSITY RESEARCH vs CLIMATE TECH examples (sustainability scope error from v1)
  - Total: 21 examples across 3 critical distinction categories
  - Problem: v1 had 85.7% FP rate (proposals, IT, biodiversity scored as sustainable tech pilots)
  - Solution: Clear negative examples showing what NOT to score as pilots

- **GATEKEEPER ENFORCEMENT STRENGTHENED**
  - Changed cap from "SET all scores = 1.0" to "SET all scores = 2.0" (more realistic)
  - Added CRITICAL NOTES section with 5 enforcement scenarios
  - Added 4 concrete examples showing gatekeeper in action
  - Clarified: deployment_maturity < 3.0 caps ALL dimensional scores at 2

- **PREFILTER v2 FIXES (see prefilter.py):**
  - Narrowed sustainability scope: Require climate/energy keywords, not generic biodiversity
  - Loosened for pilots: Added pilot-specific patterns ("pilot project", "demonstration", "field test")
  - Expected: Reduce FP rate from 85.7% → <10%, increase pass rate from 2.3% → 5-20%
...
**Token estimate**: ~3,200 tokens (v1.1 was ~2,900 tokens, added 21 distinction examples)
