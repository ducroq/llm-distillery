"""
Cultural Discovery Filter v3 - Hybrid Inference Pipeline
...
Usage:
    from filters.cultural_discovery.v3.inference_hybrid import CulturalDiscoveryHybridScorer
    ...
    # CLI
    python filters/cultural-discovery/v3/inference_hybrid.py --input articles.jsonl --output results.jsonl
"""
...
class CulturalDiscoveryHybridScorer(HybridScorer):
    """
    Two-stage hybrid scorer for cultural-discovery filter v3.

    Combines:
    - Stage 1: multilingual-e5-small embeddings + MLP probe
    - Stage 2: Existing CulturalDiscoveryScorer (Qwen2.5-1.5B fine-tuned)
    """
    ...
    def __init__(
        ...
        self._scorer_module = import_module("filters.cultural-discovery.v3.inference")
    ...
    def _get_embedding_stage_config(self) -> Dict:
        """Return EmbeddingStage configuration for cultural-discovery v3."""
        return {
...
def main():
    ...
    if args.input:
        ...
        if args.compare:
            ...
            cd_module = import_module("filters.cultural-discovery.v3.inference")
        ...
        print("\n--- Hybrid Scorer Demo ---")
        demo_article = {
            "title": "Ancient DNA Reveals Unexpected Cultural Exchange Between Viking and Byzantine Artisans",
            "content": (
                "A groundbreaking analysis of jewelry found in a 10th-century Swedish burial site "
                "reveals techniques previously thought exclusive to Byzantine craftsmen. DNA analysis "
                "of bone fragments found alongside the artifacts suggests the artisan was of mixed "
                "Scandinavian-Anatolian heritage, providing the first physical evidence of direct "
                "craft knowledge transfer between these civilizations."
            ),
        }
