...
Shared logic for InvestmentRiskScorer and InvestmentRiskScorerHub.
Eliminates code duplication between local and Hub inference.

This module provides:
- Common constants (dimensions, weights, gatekeeper thresholds)
- Tier assignment logic (simplified weighted_avg thresholds)
- Score processing (calibration, clamping, weighted average, gatekeeper)
- Input validation
...
class BaseInvestmentRiskScorer(ABC):
    """
    Abstract base class for investment risk scoring.

    Provides shared logic for both local model loading (InvestmentRiskScorer)
    and HuggingFace Hub loading (InvestmentRiskScorerHub).
    ...
    # Gatekeeper: Evidence < 4 caps overall at 2.9
    EVIDENCE_DIMENSION = "evidence_quality"
    EVIDENCE_GATEKEEPER_MIN = 4.0
    EVIDENCE_GATEKEEPER_CAP = 2.9
    ...
    def _process_raw_scores(self, raw_scores, result: Dict) -> Dict:
        ...
        # Apply gatekeeper: Evidence < threshold caps overall
        if scores[self.EVIDENCE_DIMENSION] < self.EVIDENCE_GATEKEEPER_MIN:
