"""
Sustainability Technology Filter v3 - Hybrid Inference Pipeline
...
Usage:
    from filters.sustainability_technology.v3.inference_hybrid import SustainabilityTechnologyHybridScorer

    scorer = SustainabilityTechnologyHybridScorer()
    result = scorer.score_article(article)
    # result["stage_used"] -> "stage1_low" or "stage2"

    # CLI
    python filters/sustainability_technology/v3/inference_hybrid.py --input articles.jsonl --output results.jsonl
"""
...
# Will be recalibrated after v3 training with Gemma-3-1B.
#
# Downstream users can raise this to focus on high-quality content only:
#   threshold=4.0  -> only HIGH-tier candidates reach Stage 2
#   threshold=2.5  -> mostly MEDIUM_HIGH+ candidates reach Stage 2
#   threshold=1.25 -> conservative, most MEDIUM+ pass through
...
class SustainabilityTechnologyHybridScorer(HybridScorer):
    """
    Two-stage hybrid scorer for sustainability_technology filter v3.

    Combines:
    - Stage 1: multilingual-e5-small embeddings + MLP probe
    - Stage 2: Existing SustainabilityTechnologyScorer (Gemma-3-1B fine-tuned)
    """
    ...
    def _get_embedding_stage_config(self) -> Dict:
        """Return EmbeddingStage configuration for sustainability_technology v3."""
        return {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Score articles with sustainability_technology hybrid scorer (two-stage pipeline)"
    )
    parser.add_argument(
    ...
    if args.input:
        ...
        if args.compare:
            ...
            standard_scorer = SustainabilityTechnologyScorer(
                use_prefilter=not args.no_prefilter,
            )
            start = time.time()
            ...
            for i, (h, s) in enumerate(zip(results, standard_results)):
                if s.get("tier") in ("medium", "medium_high", "high_sustainability") and h.get("stage_used") == "stage1_low":
                    disagreements += 1
                    print(
        ...
        print("\n--- Hybrid Scorer Demo ---")
        demo_article = {
            "title": "New Solar Panel Design Achieves 30% Efficiency in Field Tests",
            "content": (
                "Researchers at MIT have developed a new perovskite-silicon tandem solar cell "
                "that achieved 30.1% efficiency in real-world field tests over 6 months. "
                "The panels maintained 95% of initial efficiency, addressing the key durability "
                "concern of perovskite cells. Manufacturing costs are projected at $0.20/watt, "
                "competitive with conventional silicon panels."
            ),
        }
