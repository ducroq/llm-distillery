...
- [ ] **Extend `_is_excluded` for per-category exceptions + migrate CD v4 / uplifting v7 to base pipeline** - Deferred to ADR-019 design discussion. Refactoring agent flagged this as the architecturally-correct next move (would let CD v4 and uplifting v7 drop their custom apply_filter for the dict-iteration part), BUT it carries enough risk to warrant its own branch and explicit regression testing against sustech. Open questions: (a) reason-string convention — current migrated filters use bare reasons (no `excluded_` prefix); base's `_is_excluded` returns `excluded_<category>`. Need to either make the prefix configurable or accept a behavior change. (b) CD v4 missing `validate_article` + `check_content_length` — base would add both, fixing the regression but changing observable behavior. (c) uplifting v7's count-based `pure_speculation` block doesn't fit the dict shape regardless, so apply_filter can't go fully declarative there. Net: real benefit is reducing the 6-of-7 custom-apply_filter ratio to 4-of-7, not eliminating it. Worth doing eventually; not worth doing as a drive-by.
- [ ] **Migrate nature_recovery v2 to fully-declarative shape via `_pre_exclusion_check`** - The hook landed in this commit; NR v2 migration deferred because the bare-reason-string convention (`disaster_no_recovery` not `excluded_disaster_no_recovery`) is widespread post-#52 and would require either a base-class reason-prefix-control attr or coordinated NexusMind tagging update. Bundle with the ADR-019 work above.
