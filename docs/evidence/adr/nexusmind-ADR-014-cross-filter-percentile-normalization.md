...
For each filter, extract weighted average scores from the existing production output on sadalsuud (`~/local_dev/NexusMind/data/filtered/{filter}/filtered_*.jsonl`), filtering by `raw_weighted_average >= 4.0` (the MEDIUM threshold). Sort and store as the empirical CDF: a sorted array of (raw_score, percentile) pairs in `filters/{name}/v{N}/normalization.json`.
...
- NexusMind `filter_base_scorer.py`: apply normalization after weighted average computation. `score_scale_factor` fully removed in #112 (2026-04-06).
